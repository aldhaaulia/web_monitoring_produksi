...
class C_Laporan extends MY_Controller {
	...
	function index() {
		...
		if ($id_role == 3) {
			...
			if ($cek_submitted['success']) {
				...
				$this->load->view('backend/lap/v_lap_view', $data);
			} else {
				// form upload berkas
				$this->load->view('backend/lap/v_lap_form', $data);
			}
		} else {
			$this->load->view('backend/lap/v_lap_list', $data);
		}
	}
	...
	function result() {
		...
		if ($cek_submitted['success']) {
			...
			$this->load->view('backend/berkas/v_lap_view', $data);
		} else {
			// form upload berkas
			$this->load->view('backend/berkas/v_lap_form', $data);
		}
	}
